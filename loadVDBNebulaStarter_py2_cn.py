...
def vns_get_param_from_selected(*args):
    ...
    info[u'nebula_shader'] = vns_getAiVolumeShader(vdbtrans)
    info[u'fog_shader'] = vns_getAiVolumeShader(fogtrans)
...
vns_resolution_label = cmds.text(label=u"分辨率")
...
vns_nebula_shader_label = cmds.text(label=u"星云材质")
...
vns_fog_shader_label = cmds.text(label=u"雾气材质")
...
vns_create_button = cmds.button(label=u"创建星云", command=vns_create_nebula)
vns_get_info_button = cmds.button(label=u"从选中星云获取参数", command=vns_set_ctrls_from_selected)
vns_replace_button = cmds.button(label=u"替换选中星云", command=vns_replace_selected)
